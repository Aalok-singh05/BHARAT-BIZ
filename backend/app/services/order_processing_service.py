...
def process_customer_order(
    ...
    pre_extracted_items: List | None = None) -> Dict:
    """
    Full order processing pipeline with DB-backed OrderSession.
    """

    extracted_items = pre_extracted_items or extract_textile_order(message)
    ...
    for session_item in session.items:
        ...
        # -------------------------------------------------
        # ⭐ HANDLE MISSING/UNKNOWN MATERIAL
        # -------------------------------------------------
        if not measurement.material_name or measurement.material_name.lower() in ["unknown", "unclear", ""]:
            negotiation_response = {
                "message": (
                    "🤔 Ek item ka material samajh nahi aaya.\n"
                    "Kaunsa material chahiye? (Cotton, Silk, Polyester, etc.)"
                ),
            ...
            responses.append({
                "material": measurement.material_name or "Unknown",
        ...
        # -------------------------------------------------
        if not measurement.normalized_meters or measurement.normalized_meters <= 0:
            negotiation_response = {
                "message": (
                    f"{color or ''} {measurement.material_name} kitna chahiye?\n"
                    "Meter ya roll mein batayein. (Example: *50 meter* ya *2 roll*)"
                ),
                "next_step": "CUSTOMER_NEGOTIATION"
            }
            session_item.status = OrderItemStatus.NEGOTIATING
        ...
        if negotiation_response.get("next_step") == "CUSTOMER_NEGOTIATION":
            negotiation_required = True
...
def process_confirmed_media_order(
    db: Session,
    session,
    available_batches: List[InventoryBatchSchema]) -> Dict:
    """
    Processes an already-created order session from MEDIA_CONFIRMATION state.
    Runs inventory check on stored items without re-extracting.
    """
    ...
    if customer and customer.outstanding_balance > 0:
        last_payment = (
            db.query(CreditLedger)
            .filter(CreditLedger.customer_phone == session.customer_phone, CreditLedger.type == "payment")
            .order_by(CreditLedger.created_at.desc())
            .first()
        )

        last_activity = last_payment.created_at if last_payment else customer.created_at
        current_time = datetime.now(timezone.utc)
        ...
        if overdue_days > 7:
            ...
            if owner_phone:
                ...
                msg = (
                    f"⚠️ *Approval Needed* (Overdue)\n\n"
                    f"Customer: {customer.business_name or session.customer_phone}\n"
                    f"Overdue: ₹{float(customer.outstanding_balance)} ({overdue_days} days)\n"
                    f"Order: {item_summary}\n\n"
                    f"Reply 'YES' to approve."
                )
    ...
    # -------------------------------------------------
    # RUN INVENTORY CHECK ON SESSION ITEMS
    # -------------------------------------------------
    session.available_batches = available_batches
    ...
    for session_item in session.items:
        ...
        if not measurement.material_name or measurement.material_name.lower() in ["unknown", "unclear", ""]:
            negotiation_response = {
                "message": "🤔 Ek item ka material samajh nahi aaya.\nKaunsa material chahiye? (Cotton, Silk, Polyester, etc.)",
                "next_step": "CUSTOMER_NEGOTIATION"
            }
            session_item.status = OrderItemStatus.NEGOTIATING
            negotiation_required = True
            responses.append({"material": measurement.material_name or "Unknown", "color": color or "Pending", "response": negotiation_response})
            continue

        # HANDLE MISSING QUANTITY
        if not measurement.normalized_meters or measurement.normalized_meters <= 0:
            negotiation_response = {
                "message": f"{color or ''} {measurement.material_name} kitna chahiye?\nMeter ya roll mein batayein.",
                "next_step": "CUSTOMER_NEGOTIATION"
            }
            session_item.status = OrderItemStatus.NEGOTIATING
            negotiation_required = True
            responses.append({"material": measurement.material_name, "color": color or "Pending", "response": negotiation_response})
            continue
        ...
        if not color:
            ...
            if possible_colors:
                color_list = ", ".join(possible_colors)
                negotiation_response = {
                    "message": f"{measurement.material_name} ke liye color nahi bataya.\nAvailable: {color_list}\nKaunsa color chahiye?",
                    "next_step": "CUSTOMER_NEGOTIATION"
                }
                session_item.status = OrderItemStatus.NEGOTIATING
            else:
            ...
            inventory_result = check_inventory(measurement, available_batches, color)
            session_item.inventory_status = inventory_result["status"]
            session_item.available_meters = inventory_result["available_meters"]
            session_item.fulfilled_batches = inventory_result["fulfilled_batches"]
            ...
            negotiation_response = generate_inventory_response(measurement, inventory_result, color)
    ...
    if negotiation_required:
        sync_session_items_to_db(db, session)
        update_workflow_state(db, session.order_id, OrderState.CUSTOMER_NEGOTIATION)
        db.commit()  # Atomic commit
        return {"order_id": session.order_id, "workflow_state": OrderState.CUSTOMER_NEGOTIATION, "responses": responses}

    else:
        sync_session_items_to_db(db, session)
        update_workflow_state(db, session.order_id, OrderState.FINAL_CUSTOMER_CONFIRMATION)
        summary_message = build_final_summary(session)
        db.commit()
        return {"order_id": session.order_id, "workflow_state": session.workflow_state, "responses": responses}
